...
import { getElementProps, getElement } from "webiny-app-site-builder/editor/selectors";
            ...
            <Transition in={true} timeout={250} appear={true}>
                {state => (
                    <ElementContainer
                        ...
                        className={"webiny-sb-page-element-container"}
                    >
...
export default compose(
    ...
    withProps(({ element }) => ({
        plugin: element
            ? getPlugins("sb-page-element").find(pl => pl.elementType === element.type)
            : null
    })),
    withHandlers({
            ...
            return connectDragSource(
                <div className={"type " + typeStyle}>
                    ...
                    <div className={"element-holder"} onClick={onClick}>
                        {renderPlugins("sb-page-element-action", { element, plugin })}
                        <span>{plugin.name.replace("sb-page-element-", "")}</span>
                    </div>
                </div>
            );
        }
