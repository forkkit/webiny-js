...
import { deactivateElement } from "webiny-app-site-builder/editor/actions";
...
import { ReactComponent as NavigateBeforeIcon } from "webiny-app-site-builder/editor/assets/icons/navigate_before.svg";

const divider = "sb-page-element-settings-divider";
...
const getElementActions = (plugin: Object) => {
    ...
    return [
        ...actions,
        { plugin: getPlugin("sb-page-element-settings-advanced") },
        { plugin: getPlugin("sb-page-element-settings-save") }
    ].filter(pl => pl);
};
...
const ElementSettingsBar = pure(({ elementType, deactivateElement }) => {
    ...
    const plugin = getPlugins("sb-page-element").find(pl => pl.elementType === elementType);
    if (!plugin) {
        return null;
    }
