...
import { isPluginActive } from "webiny-app-site-builder/editor/selectors";
import { withActiveElement, withKeyHandler } from "webiny-app-site-builder/editor/components";
...
import { updateElement, deactivatePlugin } from "webiny-app-site-builder/editor/actions";
                    ...
                    {({ submit, Bind, data, form }) => (
                        <React.Fragment>
                            <DialogBody className={dialogBody}>
                                <Tabs>
                                    {renderPlugins(
                                        "sb-page-element-advanced-settings",
                                        { Bind, data, form },
...
export default compose(
    connect(
        state => ({
            open: isPluginActive("sb-page-element-settings-advanced")(state)
        }),
    ...
    withHandlers({
        closeDialog: ({ deactivatePlugin }) => () => {
            deactivatePlugin({ name: "sb-page-element-settings-advanced" });
        }
    }),
    withHandlers({
        onSubmit: ({ element, updateElement, closeDialog }) => (formData: Object) => {
            // Get element settings plugins
            const plugins = getPlugins("sb-page-element-advanced-settings").filter(
