...
import { deactivatePlugin, updateElement } from "webiny-app-site-builder/editor/actions";
import { getContent } from "webiny-app-site-builder/editor/selectors";
import { withKeyHandler } from "webiny-app-site-builder/editor/components";
...
import { ReactComponent as SearchIcon } from "webiny-app-site-builder/editor/assets/icons/search.svg";
...
import createBlockPlugin from "webiny-app-site-builder/admin/components/withSavedElements/createBlockPlugin";
...
class SearchBar extends React.Component<SearchBarProps, SearchBarState> {
    ...
    componentDidMount() {
        this.props.addKeyHandler("escape", e => {
            e.preventDefault();
            this.props.deactivatePlugin({ name: "sb-search-blocks-bar" });
        });
    }
    ...
    addBlockToContent = plugin => {
        ...
        const element = { ...content, elements: [...content.elements, plugin.create()] };
    ...
    deleteBlock = async ({ plugin, deleteElement }) => {
        ...
        const { error } = response.data.siteBuilder.deleteElement;
        if (error) {
    ...
    updateBlock = async ({ updateElement, data: { title: name, category } }) => {
        ...
        const { error, data } = response.data.siteBuilder.updateElement;
        if (error) {
            showSnackbar(error.message);
            return;
        }
    ...
    render() {
        ...
        const plugins: Object = {
            categories: {
                list: [allBlockCategory, ...getPlugins("sb-block-category")],
                active: null
            },
            blocks: getPlugins("sb-block")
        };

        plugins.categories.active = plugins.categories.list.find(({ id }) => id === active);
        const blocksList = this.getBlocksList(plugins);

        return (
            <OverlayLayout
                barMiddle={this.renderSearchInput()}
                onExited={() => this.props.deactivatePlugin({ name: "sb-search-blocks-bar" })}
            >
                <SplitView>
                    <LeftPanel span={3}>
