...
import { publishRevision, getPage } from "webiny-app-site-builder/admin/graphql/pages";
            ...
            [prop]: ({ pageDetails: { page }, gqlPublish, showSnackbar }) => async (
                ...
                const { data: res } = await gqlPublish({
                    ...
                    update: (cache, { data }) => {
                        ...
                        if (data.siteBuilder.publishRevision.error) {
                            return;
                        }
                        ...
                        cache.writeQuery({
                            query: getPageQuery,
                            data: set(pageFromCache, "siteBuilder.page.data", page)
                        });
                    }
                });

                const { error } = res.siteBuilder.publishRevision;
                if (error) {
