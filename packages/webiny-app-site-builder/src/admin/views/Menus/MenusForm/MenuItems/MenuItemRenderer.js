...
class NodeRendererDefault extends React.Component<*> {
    ...
    render() {
        ...
        const plugin = getPlugins("sb-menu-item").find(pl => pl.menuItem.type === node.type);
        if (!plugin) {
            return null;
        }

        const handle = connectDragSource(<div className="rst__moveHandle">{plugin.menuItem.icon}</div>, {
            dropEffect: "copy"
        });
