...
import { createPage, listPages } from "webiny-app-site-builder/admin/graphql/pages";
...
export default compose(
    ...
    withDataList({
        query: listPages,
        response: data => {
            return get(data, "siteBuilder.pages", {});
        },
        variables: {
            sort: { savedOn: -1 }
        }
    ...
    withHandlers({
        createPage: ({ createMutation, history, showSnackbar }) => async category => {
            try {
                const res = await createMutation({ variables: { category }, refetchQueries: ["SiteBuilderListPages"] });
                const { data } = res.data.siteBuilder.page;
